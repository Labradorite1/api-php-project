...
<body>
	...
	<main>
		<?php
			...
			{
				$apiKey = '0f21708074a04dc5bff0f41cea7bd65b'; // API key for bungie.net api
							...
							foreach($player->Response->profile->data->characterIds as $characterId) //for every character in the account
							{
								if($player->Response->characters->data->$characterId->dateLastPlayed >= $dateLastPlayedPlayer || $notOnline == true) //check if the characters date last played is the same or higher than the accounts to figure out which character the player is playing on
								{
									...
									foreach ($itemHashes as $itemHash) //foreach item hash in the list
									{
										...
										$item = $inventoryManifest->$itemHash;
										if($item->itemTypeDisplayName == "Helmet" && $item->itemTypeAndTierDisplayName != "Exotic Helmet") //check if its a helmet, dont do anything if it's not exotic
										{

										}
										elseif($item->itemTypeDisplayName == "Gauntlets" && $item->itemTypeAndTierDisplayName != "Exotic Gauntlets") //check if they are gauntleys, dont do anything if they're not exotic
										{
											
										}
										elseif($item->itemTypeDisplayName == "Chest Armor" && $item->itemTypeAndTierDisplayName != "Exotic Chest Armor")  //check if its a chestpiece, dont do anything if it's not exotic
										{
											
										}
										elseif($item->itemTypeDisplayName == "Leg Armor" && $item->itemTypeAndTierDisplayName != "Exotic Leg Armor") //check if they are boots, dont do anything if they're not exotic
										{
											
										}
										elseif($item->itemTypeDisplayName == "Titan Subclass") // checks if its a titan subclass
										{
											echo "<div class=$itemClass>"; //start item container

											$itemName  = $subclassTree . " " . $item->displayProperties->name; //define the name and add the tree to it
											echo "<h3>$itemName</h3>"; 

											$imageLink = '"' . "https://www.bungie.net/" . $item->displayProperties->icon . '"';
										...
										elseif($item->itemTypeDisplayName == "Hunter Subclass") // checks if its a hunter subclass
										{
											echo "<div class=$itemClass>"; //start item container

											$itemName  = $subclassTree . " " . $item->displayProperties->name;
											echo "<h3>$itemName</h3>";
										...
										elseif($item->itemTypeDisplayName == "Warlock Subclass") // checks if its a warlock subclass
										{
											echo "<div class=$itemClass>"; //start item container
										...
										else
										{
											...
											foreach($item->sockets->socketEntries as $socketEntry)
											{
												...
												if($perkHash != 0)
												{
													$perk = $inventoryManifest->$perkHash;
													if(property_exists($perk->displayProperties, 'icon'))
													{
														$iconUrlPerk = "https:www.bungie.net/" . $perk->displayProperties->icon;
